...
for i in range(len(t_arr)):
    ...
    ax_scatt_all.loglog(q_ana, Iq_ana, color=color_rainbow[i], zorder=len(t_arr)+i, label= 'Ana (t = {0} s)'.format(t))
    ax_scatt_all.loglog(q_num, Iq_num, color=color_rainbow[i], zorder=i, linestyle='', 
...
ax.plot(sig_eff_t, sig_eff_num, 'b', label= 'Simulation value')
ax.plot(sig_eff_t, factor*contrast_arr**2, 'r', linestyle='', marker='^', markersize=5, label= 'Fit value')
