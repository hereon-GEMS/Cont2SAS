...
# not req., only numerical SAS calculated
...
# Choose a colormap
cmap = plt.get_cmap('winter_r')  # or 'plasma', 'rainbow', etc.
...
for sim_t_idx, sim_time in enumerate(sim_times):
    ...
    length_a=float(moose_inp['length_a'][()])
    length_b=float(moose_inp['length_b'][()])
    length_c=float(moose_inp['length_c'][()])
    ...
    for i, t in enumerate(t_arr):
        ...
        for j in range(n_ensem):
            ...
            if idx_ensem==0:
                ...
                img = ax.scatter(pseudo_pos_x, pseudo_pos_y,
                                  c=pseudo_b_cat_val_xy, s=5, cmap='viridis')
                ## color bar
                cbar=plt.colorbar(img, ax=ax)  # Add colorbar to subplot 1
                cbar_label=r"Scattering length (b) [$10^{-5} \cdot \mathrm{\AA} = \mathrm{fm}$]"
                cbar.set_label(cbar_label, labelpad=10)
