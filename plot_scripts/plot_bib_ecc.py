...
def ball_in_box(qmax,qmin,Npts,scale,scale2,bg,sld_box, sld_ball,sld_sol,length_a,length_b,length_c,radius):
    ...
    for i in range(len(q_vec)):
        ...
        psi_lim=np.pi
        alpha_lim=np.pi/2 
        Aq[i]=(1/psi_lim)*gauss_legendre_double_integrate(func,[0, alpha_lim],[0, psi_lim],76)
    return (scale)*Aq, q_vec
...
# model name
sim_model= 'bib_ecc' #root.find('model').text
...
ecc_y=2
ecc_z=2
R_ecc=[ecc_x, ecc_y, ecc_z]
# dir name for model param
model_param_dir_name = ('rad' + '_' + str(ball_rad) + '_' +
                        'sld_in' + '_' + str(ball_sld) + '_' +
                        'sld_out' + '_' + str(box_sld)+ '_' +
                        'x' + '_' + str(ecc_x)+ '_' +
                        'y' + '_' + str(ecc_y)+ '_' +
                        'z' + '_' + str(ecc_z)).replace('.', 'p')
...
for i in range(len(t_arr)):
    ...
    Iq_ana,q_ana= ball_in_box_ecc(qmax=np.max(q),qmin=np.min(q),Npts=100,
                                  scale=1,scale2=1,bg=0,sld_box=box_sld,
                                  sld_ball=ball_sld,sld_sol=0,
                                  length_a=length_a, length_b=length_b, length_c=length_c,
                                  radius=ball_rad, origin_shift=R_ecc)
    ## Normalize by volume
