...
# Number of samples (colors) you want
N = 6
...
for t_idx, moose_time in enumerate(times):
    print(f'current time step: {moose_time}')
    moose_inp_file_name='{0}_{1:0>5}.h5'.format(phenm, moose_time)
    moose_inp_file=os.path.join('moose', moose_inp_file_name)
    moose_inp=h5py.File(moose_inp_file,'r')

    ### struct gen ###
    xml_dir='./xml' 
    length_a=250.#moose_inp['length_a'][()]
    length_b=250.#moose_inp['length_b'][()]
    length_c=250.#moose_inp['length_c'][()]
    nx=moose_inp['nx'][()]
    ny=moose_inp['ny'][()]
    nz=moose_inp['ny'][()]
    el_type='lagrangian'
    ...
    model_param_dir_name = ('name_' + str(name) + '_time_' + str(moose_time) \
        + '_qclean_sld_' + str(qclean_sld)).replace('.','p')
    ...
    # time array according to sim.xml
    t_arr=np.arange(0,t_end+dt, dt)
    scatt_settings='cat_' + method_cat + '_' + str(num_cat) + 'Q_' + str(Q_range[0]) + '_' + str(Q_range[1]) + '_' + 'orien__' + str(num_orientation) 
    ...
    for i in range(len(t_arr)):
        ...
        # plot
        ax.semilogx(q, Iq,color=colors[t_idx], linestyle='-', marker='none', markersize=3, label='Numerical calculation')
        ...
        q_p=q[Iq_p_arg]
        #print(q_p)
        ch_len=round(2*np.pi/q_p,2)
        from IPython.display import Math
        Math(r'2\,\mathrm{\AA}')
        AA_code = "\u212B" 
        print(f'Characteristic length is {ch_len} {AA_code} ({ch_len/10} nm)')
        # print('Q [$\mathrm{\AA}^{-1}$]')


        ax.text(0.04, 7000+t_idx*3000, 
                f"t = {moose_time}s", 
                fontsize=12, ha='center', va='center',
                bbox=dict(facecolor='yellow', edgecolor='black', boxstyle='round,pad=0.3', alpha=1))
        ## Draw arrow
        # Draw a horizontal line (rightward)
        ax.plot([0.04, 0.07], [7000+t_idx*3000, 7000+t_idx*3000], linewidth=1, color='black', linestyle='-')
        # # Draw a horizontal line (to the right)
        # ax_fit_all.plot([0, 2], [14, 14], linewidth=1, color='black', linestyle='--')
        # Draw arrowhead
        arw_x=0.07
        arw_y=7000+t_idx*3000
        dx=0.1-arw_x
        ...
        q_target = q[Iq_idx]
        dy=Iq_target-arw_y
        # ax.arrow(arw_x, arw_y, dx, dy, linewidth=1, head_width=0.0001, head_length=0.0001, 
        #                     fc='black', ec='black')
        ax.plot([0.07, q_target], [7000+t_idx*3000, Iq_target], linewidth=1, color='black', linestyle='-')
