...
def test_check_phase_field_plot():
    ...
    assert if_exist==1, "IvsQ plot for phase field model was not created"
...
def test_compare_phase_field_data():
    ...
    assert model_exist_1==1, "model dir does not exist, sim time 0"
    ### time dir 1
    t_dir_1=os.path.join(model_dir_1, 't000')
    t_exist_1=os.path.isdir(t_dir_1)
    assert t_exist_1==1, "time dir does not exist, sim time 0"
    ### data
    ...
    assert data_exist_1==1, "hdf5 file does not exist, sim time 0"
    ...
    assert model_exist_2==1, "model dir does not exist, sim time 86400"
    ### time dir 2
    t_dir_2=os.path.join(model_dir_2, 't000')
    t_exist_2=os.path.isdir(t_dir_2)
    assert t_exist_2==1, "time dir does not exist, sim time 86400"
    ### data
    #### num data (read + check exist)
    ...
    assert data_exist_2==1, "hdf5 file does not exist, sim time 86400"
    ...
    ## sim time = 0s
    assert q_num_1 == pytest.approx(q_gold_1, abs=1e-6), "Q values do not match, t=0s"
    assert Iq_num_1 == pytest.approx(Iq_gold_1, abs=1e-6), "IQ values do not match, t=0s"
    ## sim time = 86400s
    assert q_num_2 == pytest.approx(q_gold_2, abs=1e-6), "Q values do not match, t=86400s"
    assert Iq_num_2 == pytest.approx(Iq_gold_2, abs=1e-6), "IQ values do not match, t=86400s"
