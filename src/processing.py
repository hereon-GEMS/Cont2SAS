...
def categorize_prop(prop,t,ndiv):
    """
    cat_prop= np.zeros(len(prop))
    cat_idx=[]
    prop_min=np.min(prop)
    prop_max=np.max(prop)
    step = (prop_max - prop_min)/ndiv
    cat_arr=np.linspace(prop_min-step/2, prop_max+step/2, ndiv+1)
    for j in range(len(cat_arr)-1):
        index_low=np.where(prop>=(cat_arr[j]))
        index_high=np.where(prop<(cat_arr[j+1]))
        # idx
        cat_idx.append(np.intersect1d(index_high,index_low))
    cat_vals=np.linspace(prop_min,prop_max,ndiv)
    for j in range(ndiv):
        print(j)
        print(cat_idx[j])
        print(cat_prop[0,cat_idx[j]])
        cat_prop[0,cat_idx[j]]=cat_vals[j]
    ...
    for i in range(len(t)):
        prop_t=prop[i]
        prop_min=np.min(prop_t)
        prop_t_max=1#np.max(prop_t)
        step = (prop_t_max - prop_t_min)/ndiv
        cat_arr=np.linspace(prop_t_min-step/2, prop_t_max+step/2, ndiv+1)
        # step = (in_arr_max - in_arr_min)/ndiv
        cat_idx_t=[]
